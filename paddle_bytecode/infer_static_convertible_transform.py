...
class InferIsProcedureStaticConvertibleTransform:
  ...
  def StatementListNode(self, ast_node):
    ...
    for child in ast_node.children:
      ...
      is_procedure_static_convertible = (
        is_procedure_static_convertible and child.data.is_procedure_static_convertible
      )
    ast_node.data.is_procedure_static_convertible = is_procedure_static_convertible

  def StatementNode(self, ast_node):
    is_procedure_static_convertible = True
    self.infer(ast_node.expr_node)
    is_procedure_static_convertible = (
      is_procedure_static_convertible and ast_node.expr_node.data.is_procedure_static_convertible
    )
    for store_instruction_tuple in ast_node.store_instructions:
      if len(store_instruction_tuple) == 1:
        # example0: `a = foo()`
        store_instruction_tuple[0].data.is_procedure_static_convertible = True
      elif len(store_instruction_tuple) == 2:
        # example0: `a[bar()] = foo()`
        # example1: `a.bar = foo()`
        self.infer(store_instruction_tuple[0])
        is_procedure_static_convertible = (
          is_procedure_static_convertible and store_instruction_tuple[0].data.is_procedure_static_convertible
        )
        store_instruction_tuple[1].data.is_procedure_static_convertible = True
      else:
  ...
  def InstructionNode(self, ast_node):
    ast_node.data.is_procedure_static_convertible = self.get_is_procedure_static_convertible(ast_node)

  def LOAD_CONST(self, ast_node):
    ast_node.data.is_procedure_static_convertible = True
...
class InferIsResultStaticConvertibleTransform:
  ...
  def StatementListNode(self, ast_node):
    ...
    ast_node.data.is_result_static_convertible = ()
  ...
  def StatementNode(self, ast_node):
    ...
    for i, store_instruction_tuple in enumerate(ast_node.store_instructions):
      if len(store_instruction_tuple) == 1:
        ...
        store_instruction.data.is_result_static_convertible = ()
        if store_instruction.instruction.opname == "STORE_FAST":
          ...
          self.store_is_local_var_static_convertible(
            store_instruction, ast_node.expr_node.data.is_result_static_convertible[i]
          )
      elif len(store_instruction_tuple) == 2:
        # example0: `a[bar()] = foo()`
        # example1: `a.bar = foo()`
        ...
        store_instruction_tuple[1].data.is_result_static_convertible = ()
      else:
        raise NotImplementedError()
  ...
  def ExpressionNode(self, ast_node):
    ...
    ast_node.data.is_result_static_convertible = last_child.data.is_result_static_convertible

  def InstructionNode(self, ast_node):
    ast_node.data.is_result_static_convertible = self.get_is_result_static_convertible(ast_node)

  def LOAD_CONST(self, ast_node):
    ast_node.data.is_result_static_convertible = (True,)
